...
class SpaceShip:
    def __init__(self, name: str = 'AlienTissue'):
        self.name = name
    ...
    def run_commot_(self, radius=350):
        ...
        with open('output/input_data/communication.pkl', 'wb') as f:
        ...
        df.to_parquet('output/input_data/LRs.parquet')
    ...
    def spawn_worker(
        ...
        outlog = 'output/logs/training_{}.log'.format(str(time.strftime("%Y%m%d_%H%M%S")))
    ...
    def run_spacetravlr(
        ...
        base_dir = 'output/betadata/'
        adata = sc.read_h5ad('output/input_data/_adata.h5ad')
        links = pickle.load(open('output/input_data/celloracle_links.pkl', 'rb'))
    ...
    def is_everything_ok(self) -> bool:
        assert os.path.isfile('output/input_data/_adata.h5ad'), "AnnData file not found"
        _adata = sc.read_h5ad('output/input_data/_adata.h5ad')
        _links = pickle.load(open('output/input_data/celloracle_links.pkl', 'rb'))
        ...
        assert os.path.isdir('output'), "Output directory not found"
        assert os.path.isdir('output/betadata'), "Betadata directory not found"
        assert os.path.isdir('output/input_data'), "Input data directory not found"
        assert os.path.isfile('output/input_data/celloracle_links.pkl'), "Base links file not found"
        assert os.path.isdir('output/logs'), "Logs directory not found"
